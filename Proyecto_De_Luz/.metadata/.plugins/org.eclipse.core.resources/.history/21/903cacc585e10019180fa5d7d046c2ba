package com.starbucks.sistema.venta.logica.Codigo;
...
import com.starbucks.sistema.venta.logica.Codigo.SLConfig.Tile;
 ...
 * components, and the optional delays that can be applied to the components to
...
public class SLKeyframe {
	 ...
	 * layout configuration. It doesn't account for any optional delay you may
	 * add to it.
	 */
	 ...
	 * slide from. A new component is a component that is present in this layout
	 * configuration but not on the previous one.
	 */
	...
	/**
	 * Sets the side of the screen from where every new component will slide
	 * from. A new component is a component that is present in this layout
	 * configuration but not on the previous one.
	 */
	...
	/**
	 * Sets the side of the screen from where every old component will slide to.
	 * An old component is a component that is not present in this layout
	 * configuration but was on the previous one.
	 */
	...
	/**
	 * Sets the delay used by the given components before the transition from
	 * their current place to their target one starts. The delay is increased
	 * for each component in the list.
	 */
	public SLKeyframe setDelayIncr(float delay, Component... cmps) {
		float d = 0;
		for (Component c : cmps)
			delays.put(c, d += delay);
		return this;
	}
	...
	public SLKeyframe addDelay(float delay, Component... cmps) {
		for (Component c : cmps)
			delays.put(c, delays.get(c) + delay);
		return this;
	}
	...
	public SLKeyframe addDelayIncr(float delay, Component... cmps) {
		float d = 0;
		for (Component c : cmps)
			delays.put(c, delays.get(c) + (d += delay));
		return this;
	}
